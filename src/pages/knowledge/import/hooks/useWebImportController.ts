...
export const useWebImportController = ({
  ...
  const [batchFrequencyValue, setBatchFrequencyValue] =
    useState(webUpdateFrequency);
  const [batchParseLoading, setBatchParseLoading] = useState(false);
  const [batchParseResultCount, setBatchParseResultCount] = useState<
    number | null
  >(null);

  const getCurrentWebUrls = () =>
    (form.getFieldValue('webUrls') as
      | ImportFormValues['webUrls']
      | undefined) ?? [];

  const getCurrentWebUpdateFrequency = () =>
    (form.getFieldValue('webUpdateFrequency') as
      | ImportFormValues['webUpdateFrequency']
      | undefined) ?? 'manual';
  ...
  const handleAddUrlRow = () => {
    ...
    if (currentWebUrls.length >= MAX_WEB_SINGLE_URL_COUNT) {
      messageApi.warning(
        `逐个上传模式下最多添加${MAX_WEB_SINGLE_URL_COUNT}条 URL`,
      );
      return;
    }
    ...
    setWebUrls([
      ...currentWebUrls,
      createEmptyWebImportItem(currentUpdateFrequency),
    ]);
  };
  ...
  const handleUpdateUrlValue = (id: string, value: string) => {
    setWebUrls(
      updateWebUrlItem(getCurrentWebUrls(), id, {
        url: value,
        parseStatus: 'idle',
      }),
    );
  ...
  /** 更新单条 URL 的更新频率。 */
  const handleUpdateUrlFrequency = (
    id: string,
    value: ImportFormValues['webUpdateFrequency'],
  ) => {
    setWebUrls(
      updateWebUrlItem(getCurrentWebUrls(), id, {
  ...
  const handleRemoveUrl = (id: string) => {
    setWebUrls(
      ensureSingleModeRows(
        removeWebUrl(getCurrentWebUrls(), id),
        getCurrentWebUpdateFrequency(),
      ),
    );
  };
  ...
  const handleConfirmApplyFrequency = () => {
    setWebUrls(
      updateWebUrlsFrequency(getCurrentWebUrls(), batchFrequencyValue),
    );
  ...
  const handleParseWebUrls = () => {
    const currentWebUrls = getCurrentWebUrls();
    const validItems = currentWebUrls.filter(
      (item) => item.url.trim() && isValidWebUrl(item.url.trim()),
    );
    ...
    void Promise.all(
      ...
      if (failureCount > 0) {
        messageApi.warning(
          `解析完成，成功${successCount}条，失败${failureCount}条`,
        );
        return;
      }
