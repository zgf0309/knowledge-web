...
const KnowledgeDocumentPage = () => {
	...
	const locationState = location.state as { record?: KnowledgeFileRecord } | undefined;
	...
	const handleSubmitChunk = async () => {
		try {
			...
			if (ok) {
				...
				messageApi.error(chunkEditor.editingId ? '切片更新失败' : '切片创建失败');
			}
		} catch (err) {
	...
	const handleOpenEditInsight = (insightId: string) => {
		const target = controller.visibleInsights.find((item) => item.id === insightId);
		if (!target) return;
	...
	const handleSubmitInsight = async () => {
		try {
			...
			const ok = await api.mutations.submitInsight(controller.currentChunkId, action, trimmed);
			if (ok) {
				messageApi.success(action === 'update' ? '知识点已更新' : '知识点已创建');
				insightEditor.close();
				api.refetchChunks();
			} else {
				messageApi.error(action === 'update' ? '知识点更新失败' : '知识点创建失败');
			}
		} catch (err) {
	...
	const handleDeleteInsight = (insightId: string) => {
		...
		modal.confirm({
			...
			okButtonProps: { danger: true },
			onOk: async () => {
				const ok = await api.mutations.submitInsight(target.chunkId, 'delete', insightId);
				if (ok) {
		...
		<PageContainer className="knowledge-document-page" title={false}>
			...
			<Flex style={{ height: 'calc(100vh - 200px)' }} gap={12}>
				<div className="knowledge-document-page__layout">
					<DocumentPreviewPanel reportContent={api.reportContent} loading={api.mdcontentQuery.isFetching} />
					<DocumentChunkPanel
						...
						onEditChunk={(chunk) => chunkEditor.openEdit(chunk.id, { content: chunk.content })}
