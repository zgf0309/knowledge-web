...
import { Button, Empty, Flex, Input, Pagination, Segmented, Select, Spin, Switch, Tag, Tooltip, Typography } from 'antd';
		...
		<Flex style={{ padding: '0 20px', flex: 'none' }}>
			<Segmented
				...
				options={[
					...
					{ label: `原文切片 (${sourceSummary.original})`, value: 'original' },
						...
						>
							<Flex justify="space-between" align="center" gap={12} className="knowledge-document-page__chunk-meta" wrap>
								<Flex align="center" gap={8} wrap>
									...
									<span className="knowledge-document-page__chunk-info"> · {CHUNK_TYPE_LABEL[item.chunk_type]} · {item.charCount}字符</span>
								</Flex>
								<Flex align="center" gap={4} onClick={(event) => event.stopPropagation()}>
									...
									<Switch size="small" checked={item.available} onChange={(checked) => onToggleChunk(item.id, checked)} />
								</Flex>
							</Flex>
							...
							<Flex justify="space-between" align="center" gap={12} className="knowledge-document-page__chunk-footer" wrap>
								<Tag color={item.available ? 'success' : 'default'}>{item.statusText}</Tag>
