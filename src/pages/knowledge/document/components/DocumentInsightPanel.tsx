...
const DocumentInsightPanel = ({
  ...
  <section className="knowledge-document-page__panel knowledge-document-page__panel--full-height">
    ...
    >
      <Flex vertical gap={4}>
        <div className="knowledge-document-page__panel-title">
          切片知识点（{visibleInsights.length}）
        </div>
        {showSubtitle ? (
      ...
      {visibleInsights.length ? (
        visibleInsights.map((item) => (
          <div key={item.id} className="knowledge-document-page__insight-card">
            <p className="knowledge-document-page__insight-content">
              {item.content}
            </p>
            <Flex justify="space-between" align="center" gap={12} wrap>
              <Flex
                ...
                gap={12}
                wrap
                className="knowledge-document-page__insight-footer"
              >
                <span>
                  来源：{item.source === 'original' ? '原文切片' : '自定义切片'}
                </span>
              </Flex>
