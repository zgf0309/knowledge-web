...
import { DEFAULT_GROUPS, INITIAL_BATCH_MOVE_STATE } from './list/constants';
import type {
  KnowledgeBaseRecord,
...
const KnowledgeListPage = () => {
  ...
  const [batchMoveState, setBatchMoveState] = useState<KnowledgeBatchMoveState>(
  ...
  const knowledgeRecords = useMemo<KnowledgeBaseRecord[]>(
    () => knowledgeList?.list ?? emptyRecords,
    [emptyRecords, knowledgeList?.list],
  );
  ...
  const allCurrentPageSelected =
    ...
    currentPageKeys.every((key: string) => selectedRowKeys.includes(key));
  const partialCurrentPageSelected =
    currentPageKeys.some((key: string) => selectedRowKeys.includes(key)) &&
  ...
  const handleToggleSelectAllCurrentPage = (checked: boolean) => {
    if (checked) {
      setSelectedRowKeys((currentKeys) =>
        Array.from(new Set([...currentKeys, ...currentPageKeys])),
      );
      return;
    }
    setSelectedRowKeys((currentKeys) =>
  ...
  return (
    <PageContainer
      ...
      className="knowledge-base-list-page"
    >
      ...
      <Flex vertical>
        <Flex align="center" justify="space-between">
          ...
          <Button
            icon={<AimOutlined />}
            className="knowledge-base-list__test-button"
          >
            命中测试
          </Button>
        </Flex>
