...
const hasTriggeredAutoLogout = (): boolean =>
  Boolean(getAuthWindow()?.[AUTO_LOGOUT_FLAG]);
...
const clearStoredAccessToken = (): void => {
  removeLocalStorage(`${NAME_SPACE}${StorageKeys.ACCESS_TOKEN}`);
};
...
const handleRequestError = (error: any): void => {
  ...
  if (error?.response?.status === 401) {
    console.warn(
      `Authentication failed (${error?.response?.status}), auto logout.`,
    );
    triggerAutoLogout('登录已失效，请重新登录');
  }
